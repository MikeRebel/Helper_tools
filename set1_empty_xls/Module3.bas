...
'        .Calculation = xlCalculationManual
    ...
    MyRowCount = 1
                If MyRowCount = 1 Then
                    ...
                    For x = 1 To ColumnsCount
                    ActiveSheet.Columns(x).Select
...
Sub Copy_Sheet_From_Folder()
    ...
    Dim ColumnsCountInput As Integer
    ...
    ColumnsCountInput = InputBox("Сколько колонок копировать?", "Выбор количества колонок для копирования", 25)

    Get_Sheet _
            PasteAsValues:=True, _
            StartColumnIndex:=0, _
            ColumnsCount:=ColumnsCountInput, _
            SourceShName:="", _
            SourceShIndex:=1, _
            myReturnedFiles:=myFiles
